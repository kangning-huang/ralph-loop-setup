...
You are helping the user set up the 3 files needed to run a Ralph Wiggum Loop - an automated task implementation system that uses AI coding assistants to work through a project todo list.
...
- `~/.gemini/skills/ralph-loop-setup/resources/ralph_wiggum_loop.sh` - The main loop script
- `~/.gemini/skills/ralph-loop-setup/resources/todolist-template.json` - Template for todo list
- `~/.gemini/skills/ralph-loop-setup/resources/progress-template.txt` - Template for progress log
...
3. **ralph_wiggum_loop.sh** - Orchestration script: runs the loop, launching fresh AI sessions
...
4. **Technical Details**:
   ...
   - "Any specific coding patterns or conventions the AI should follow?"
...
```bash
cp ~/.gemini/skills/ralph-loop-setup/resources/ralph_wiggum_loop.sh ./ralph_wiggum_loop.sh
chmod +x ./ralph_wiggum_loop.sh
```

#### File 2: Create todolist.json

Read the template from `~/.gemini/skills/ralph-loop-setup/resources/todolist-template.json` for reference, then create a customized version with the user's tasks:
...
- Run a fresh AI session to implement it
...
4. **Write Clear Acceptance Criteria**: How will the AI know when it's done?
5. **Hint at Files**: The `files_likely_affected` field helps the AI focus
